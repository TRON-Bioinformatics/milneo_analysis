...
cohorts_of_interest <- c("hugo", "vanallen", "riaz_pre", "miao", "mcdermott_Atezo")
...
randomise_df <- function(df, n = 100){
  ...
  df_random <- as_tibble(transform(df, id = l_ids[[n]]))
  return(df_random)
}
...
set.seed(42)
dat_features_full_random <- randomise_df(dat_features_full, n = 10000)
dat_features_full_random <- dat_features_full_random %>%
  select(-binary_response)%>%
  left_join(df_response, by = "id")

# export data
write_csv(dat_features_full_random, paste0(path_snv_out, date_today, "randomised_data.csv"))
...
# export data
write_csv(dat_features_indel_full_random, paste0(path_indel_out, date_today, "randomised_data.csv"))
...
# export data
write_csv(fusion_full_random, paste0(path_fusion_out, date_today, "randomised_data.csv"))
...
write_csv(combined_full_random, paste0(path_combined_out, date_today, "randomised_data.csv"))
